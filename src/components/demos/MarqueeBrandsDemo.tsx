    ...
    <>
      <Marquee className="[--duration:20s] p-0" pauseOnHover>
        {brandList.map((brand, index) => (
          <div key={index}>
            {/* Displaying lightimg in light mode and image in dark mode */}
            <img
              src={brand.image}
              alt={brand.name}
              className="hidden dark:block w-36 h-8 mr-6 lg:mr-20"
            />
            <img
              src={brand.lightimg}
              alt={brand.name}
              className="w-36 h-8 mr-12 lg:mr-20 dark:hidden"
            />
          </div>
        ))}
      </Marquee>
