...
export function ScrollVelocityMarqueeDemo() {
  ...
  return (
    <div
      className="flex w-full h-[600px] relative justify-center items-center flex-col bg-black overflow-hidden rounded-2xl"
      ref={container}
    >
      <h1 className="absolute text-center text-3xl sm:text-5xl md:text-6xl top-1/3 sm:top-1/3 md:top-1/4 text-white font-calendas z-10 pointer-events-none select-none drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)]">
        Weekly Finds
      </h1>
      <div className="absolute h-[150%] sm:h-[180%] top-0 w-full justify-center items-center flex flex-col space-y-2 sm:space-y-3 md:space-y-4">
        <SimpleMarquee
          className="w-full"
          baseVelocity={8}
          repeat={4}
          ...
          {firstThird.map((src, i) => (
            <MarqueeItem key={i}>
              <img
                src={src}
                alt={`Image ${i + 1}`}
                className="h-20 w-32 sm:h-24 sm:w-40 md:h-32 md:w-48 object-cover rounded-xl border border-zinc-800"
              />
            </MarqueeItem>
          ))}
        </SimpleMarquee>
...
export default ScrollVelocityMarqueeDemo;
