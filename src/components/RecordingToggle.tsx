...
import "./RecordingToggle.css";

export function RecordingToggle({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <div className={`recording-toggle-wrapper ${dark ? "dark-mode" : ""} scale-[1.6] md:scale-[2.0] origin-center my-6 md:my-10`}>
      <button
        className="recorder"
        type="button"
      ...
      >
        <span className="recorder__label-start" aria-hidden={recording}>
          Stop
        </span>
        <span className="recorder__switch">
          <span className="recorder__switch-handle">
            <svg
              className="recorder__timer"
              viewBox="0 0 16 16"
              width="16px"
              height="16px"
              aria-hidden="true"
            >
              ...
              >
                <circle
                  className="recorder__timer-ring"
